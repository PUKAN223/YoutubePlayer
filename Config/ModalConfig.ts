...
import { client, connection, queue } from "../Music"
import { HasSongEmbed, command, volumeMap } from "./CommandConfig"

export const SongMap = new Map<string, { user: User, channel: VoiceBasedChannel, author: string }>()
...
async function listen(guild: Guild, channel: VoiceBasedChannel, links: string, interaction: ModalSubmitInteraction) {
    ...
    if (queue() == undefined) {
        ...
        if (channel.guildId == guild.id) {
            ...
            SongMap.set(song.id as string, { user: interaction.user, author: song.uploader.name as string, channel: channel })
